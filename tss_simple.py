...
class Tesseract_GUI(tk.Frame):
    ...
    def configure_grid(self):
        ...
        self.instructions = tk.Frame(master=self.parent, bg='snow', bd=2, relief=tk.GROOVE)
        ...
        self.statusbar = tk.Frame(master=self.parent, bg='snow', bd=2, relief=tk.GROOVE, height=12)
        # Configure 'em
    ...
    def create_widgets(self):
        ...
        self.button_choice = tk.Button(master=self.buttons, text='Choose File', command=self.choose_file, width=12)
        self.file_name_label = tk.Entry(master=self.buttons, state='disabled', exportselection=0, width=24, textvariable=self.selected)
        self.button_convert = tk.Button(master=self.buttons, text='OCR', command=self.convert, width=12)
        self.output = scrolledtext.ScrolledText(master=self.textbox)
        self.output.insert(tk.END, "Converted text will display here")
        self.status_label = tk.Label(master=self.statusbar, text=self.status, anchor='w')

    def widget_geometry(self):
        self.instruction_label.pack(fill=tk.BOTH, padx=2, pady=2, expand=True)
        self.button_choice.pack(side='left', padx=2, pady=2, ipadx=2, ipady=2, anchor=tk.CENTER)
        self.file_name_label.pack(side='left', padx=10, pady=2, ipadx=2, ipady=2, expand=True, anchor=tk.CENTER)
        self.button_convert.pack(side='left', padx=2, pady=2, ipadx=2, ipady=2, anchor=tk.CENTER)
    ...
    def get_status(self):
        if self.status is None:
            ...
            if which('tesseract') is None:  # shutil.which
                self.status = 'Status: ' + 'Did not found a valid Tesseract Installation'
    ...
    def choose_file(self):
        ...
        self.filename = filedialog.askopenfilename(
            initialdir=".",
            title = "Select file",
            filetypes = (("Image Files", "*.png *.jpg *.jpeg *.tiff"), ("All Files","*.*"))
            )
        if self.filename:
        ...
        if not Path('{}'.format(file_path)).is_file():
            ...
            with open(file_path, 'r') as settings_file:
                try:
                    ...
                    self.set_status('Language-File Importing-Error {} - using english & utf-8'.format(err))
                    return "eng", "utf-8"
    ...
    def convert(self):
        ...
        if self.filename and self.tesseract:
            ...
            try:
                ...
                subprocess.run(process, stdout=subprocess.DEVNULL)  # TODO: catch output and exceptions
